...
int main()
{
    FILE *fp = fopen("inputs/everybody_codes_e2025_q02_p3.txt", "r");
    //FILE *fp = fopen("inputs/everybody_codes_e2025_q02_p3-test.txt", "r");
    ...
    for (int row = row_first; row <= row_last; row += 1) {
        for (int point = point_first; point <= point_last; point += 1) {
            coord.x = point;
            coord.y = row;
            result.x = 0;
            result.y = 0;
